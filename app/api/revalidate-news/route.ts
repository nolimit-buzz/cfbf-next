...
import { NEWS_CACHE_TAG } from '@/lib/cms/news';

/**
 * Webhook target for Strapi: purges the cached News page when an editor
 * publishes. Point the CMS at POST /api/revalidate-news with the shared
...
export async function POST(request: Request) {
  ...
  return Response.json({ revalidated: true, tag: NEWS_CACHE_TAG });
}
